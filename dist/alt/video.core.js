...
(function (global, factory) {
  ...
  function (_Slider) {
    ...
    _proto.handleMouseMove = function handleMouseMove(event) {
      ...
      var liveTracker = this.player_.liveTracker; // Immediately convert to a percentage for css value when mouse move

      this.requestAnimationFrame(function () {
        // Set the new bar width or height
        var sizeKey = _this3.vertical() ? 'height' : 'width'; // Convert to a percentage for css value

        _this3.bar.el().style[sizeKey] = (distance * 100).toFixed(2) + '%';
      });
